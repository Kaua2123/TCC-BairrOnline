...
CREATE TABLE IF NOT EXISTS `bairronline`.`denuncias` (
  ...
  CONSTRAINT `fk_denuncias_denunciante1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_denuncias_imagem1`
    FOREIGN KEY (`imagem_img_id`)
    REFERENCES `bairronline`.`imagem` (`img_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
